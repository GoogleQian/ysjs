    ...
    <el-dialog :title="'优惠券'+dialogTitle" :visible.sync="dialogVisible">
      <el-form :model="dialogData" ref="ruleForm" label-width="120px" class="demo-ruleForm">
        ...
        <el-form-item label="开始时间">
          ...
          <el-date-picker v-model="dialogData.start_time" type="datetime" placeholder="选择日期"
                          value-format="yyyy-MM-dd HH:mm:ss">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="结束时间">
          <!-- <el-input v-model="dialogData.end_time"></el-input> -->
          <el-date-picker v-model="dialogData.end_time" type="datetime" placeholder="选择日期"
                          value-format="yyyy-MM-dd HH:mm:ss">
          </el-date-picker>
        </el-form-item>
    ...
    methods: {
      ...
      del(row) {
        this.$confirm("此操作将永久删除该条数据, 是否继续?", "提示", {
          ...
          .then(() => {
            delCoupons({couponId: row.id}).then(res => {
              this.$message({
