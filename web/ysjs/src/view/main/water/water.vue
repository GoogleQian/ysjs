...
import { getHistoryInfo } from "@/service/api/waterQueality";
export default {
  data() {
    return {
      tableData: [],
  ...
  methods: {
    ...
    search(searchForm) {
      ...
      this.searchForm.start_time = searchForm.daterange[0];
      this.searchForm.end_time = searchForm.daterange[1];
      delete this.searchForm.daterange;
      this.getList();
    },
    func({ opera, row }) {
      console.log(opera);
